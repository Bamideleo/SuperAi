...
@section('content')
    {{-- main content --}}
    <div
        class="dark:bg-[#292929] bg-[#F6F3F2] sidebar-scrollbar overflow-auto flex flex-col flex-1 font-Figtree border-l dark:border-[#474746] border-color-DF h-screen">
                            ...
                            <tbody class="gallery">
                                @forelse ($images as $image)
                                    <tr class="gallery-item border-b dark:border-[#474746]" id="image_{{ $image->id }}">
                                        <td
                                            class="pl-3 lg:pl-[34px] 2xl:w-[224px] lg:w-[124px] flex py-5 image-table-row-one">
                                            <span
                                                class="justify-center w-[52px] h-[52px] lg:h-16 lg:w-16 flex items-center">
                                        ...
                                        <td
                                            class="text-14 font-Figtree py-[18px] text-color-14 dark:text-white font-medium md:pl-[34px] md:pr-6 px-3 docs-table-row-one">
                                            <p
                                                class="w-[138px] xs:w-[191px] lg:w-[200px] 9xl:w-[300px] 6xl:w-[200px] break-words line-clamp-double">
                                                {{ $image->title }}
                                            </p>
                                            <p class="xl:hidden mt-2 break-words">
                                                {{ empty($image->provider) ? '--' : ucFirst($image->provider) }}</p>
                                            <p class="xl:hidden mt-2 break-words">
                                                {{ timeZoneFormatDate($image->created_at) }}</p>
                                        </td>
                                        <td
                                            class="text-13 font-Figtree py-[22px] text-color-89 font-medium px-6 w-64 hidden xl:table-cell">
                                            {{ empty($image->provider) ? '--' : ucFirst($image->provider) }}
                                        </td>
                                        <td
                                            class="text-13 font-Figtree py-[22px] text-color-89 font-medium px-6 w-64 hidden xl:table-cell whitespace-nowrap">
                                        ...
                                        <td
                                            ...
                                            <div class="gap-4 justify-end items-center hidden xl:flex">
                                                <div class="relative">
                                                    <a class="image-modal-button image-tooltip-view flex items-center border border-color-89 dark:border-color-47 text-color-14 dark:text-white bg-white dark:bg-color-47 p-2 rounded-lg justify-center"
                                                        title="{{ __('View :x', [ 'x' => $image->type == 'video' ? __('Video') : __('Image') ]) }}" href="javascript:void(0)"
                                                        data-name="{{ $image->title }}" data-promt="{{ $image->title }}"
                                                        data-id="{{ $image->id }}"
                                                        ...
                                                        data-advanced_created_on="{{ $image->created_at }}"
                                                        data-is-favorite="{{ in_array($image->id, $userFavoriteImages) ? 'true' : 'false' }}"
                                                        data-source="{{ $image->imageUrl(['thumbnill' => true, 'size' => 'medium']) }}"
                                                        id={{ $image->id }} data-slug="{{ $image->slug }}">
                                                ...
                                                <div class="relative">
                                                    <a class="file-need-download image-tooltip-download flex items-center border border-color-89 dark:border-color-47 text-color-14 dark:text-white bg-white dark:bg-color-47 p-2 rounded-lg justify-center"
                                                        title="{{ __('Download') }}" href="{{ $image->type == 'video' ? $image->videoUrl() : $image->imageUrl() }}"
                                                ...
                                                <div class="relative">
                                                    <a class="image-tooltip-delete flex items-center border border-color-89 dark:border-color-47 text-color-14 dark:text-white bg-white dark:bg-color-47 p-2 rounded-lg justify-center modal-toggle"
                                                        title="{{ __('Delete Item') }}" href="javascript: void(0)"
                                            ...
                                            <div class="relative xl:hidden inline-block">
                                                ...
                                                <div
                                                    class="absolute ltr:right-0 rtl:left-0 mt-2 w-[201px] border border-color-89 dark:border-color-47 rounded-lg bg-white dark:bg-[#333332] z-50 table-drop-body dropdown-shadow table-dropdwon-rtl">
                                                    <div>
                                                        <a href="javascript:void(0)"
                                                            class="image-modal-button flex justify-start items-center gap-1.5 text-14 font-normal text-color-14 dark:text-white font-Figtree px-4 py-2 hover:bg-color-F6 dark:hover:bg-[#3A3A39] rounded-t-lg text-left"
                                                            data-name="{{ $image->title }}"
                                                            data-promt="{{ $image->title }}"
                                                            data-id="{{ $image->id }}"
                                                            data-style="{{ data_get($image->generation_options, 'art_style', 'None') }}"
                                                            data-resulation="{{ data_get($image->generation_options, 'size', 'None') }}"
                                                            data-created="{{ $image->created_at }}"
                                                            data-lightstyle="{{ data_get($image->generation_options, 'light_effect', 'None') }}"
                                                            data-is-favorite="{{ in_array($image->id, $userFavoriteImages) ? 'true' : 'false' }}"
                                                            data-source="{{ $image->imageUrl(['thumbnill' => true, 'size' => 'medium']) }}"
                                                            id={{ $image->id }} data-slug="{{ $image->slug }}">
                                                            <span class="w-4 h-4">
                                                                <svg class="w-4 h-4" width="16" height="10"
                                                            ...
                                                            <p>{{ __('View :x', [ 'x' => $image->type == 'video' ? __('Video') : __('Image') ]) }}</p>
                                                        </a>
                                                        <a href={{ $image->imageUrl() }} download={{ $image->title }}
                                                            class="flex justify-start items-center gap-1.5 text-14 font-normal text-color-14 dark:text-white font-Figtree px-4 py-2 hover:bg-color-F6 dark:hover:bg-[#3A3A39] text-left">
                                                            <span class="w-4 h-4">
                                                        ...
                                                        <a href="javascript: void(0)" id="{{ $image->id }}"
                                                            ...
                                                            <span class="w-4 h-3">
                                                                <svg class="w-4 h-3" width="11" height="12"
                                ...
                                @empty
                                    <tr class="border-b dark:border-[#474746]">
                                        <td colspan="5" class="w-full">
                                            <p
                                                class="text-center font-Figtree text-16 font-medium text-color-14 py-5 dark:text-white">
                                                {{ __('No image is available to be displayed.') }}</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
