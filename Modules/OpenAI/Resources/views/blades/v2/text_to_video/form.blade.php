...
@endphp
<form id="textToVideoForm" enctype='multipart/form-data'>
    <div class="px-5 py-[22px] sm:py-8 xl:p-6 xl:pb-[56px] pt-14 font-Figtree">
        <div class="bg-white dark:bg-[#474746] py-7 px-6 rounded-xl mt-5">
            <p class="text-color-14 text-24 font-semibold font-RedHat dark:text-white">{{ __('Create videos like never before!') }}</p>
            ...
            <div class="flex flex-col mt-6" data-field="prompt">
                ...
                <textarea
                    ...
                    id="video-description" placeholder="{{ __('Briefly write down the description of the image you have in mind..') }}" required oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')" rows="3" name="prompt">{{ $prompt ?? '' }}</textarea>
            </div>
                    ...
                    <div class="font-normal custom-dropdown-arrow text-14 text-color-2C dark:text-white">
                        ...
                        <select class="select block w-full text-base leading-6 font-medium text-color-2C bg-white bg-clip-padding bg-no-repeat dark:bg-[#333332] rounded-xl dark:rounded-2xl m-0 focus:text-color-2C focus:bg-white focus:border-color-89 focus:outline-none" id="provider" name="provider">
                            @foreach ( $aiProviders as $provider => $value )
                                @php
                                    $providerName = str_replace('texttovideo_', '', $provider);
                                @endphp
                        ...
                        @if (!empty($providerOptions))
                            ...
                            <div class="gap-6 pt-3 grid grid-cols-2 ProviderOptions {{ $providerName . '_div' }}">
                                @foreach ($fields as $field)
                                    ...
                                    <div {{ isset($field['type']) ? 'data-field=' . $field['type'] : '' }} >
