...
class TextToVideoDataTable extends DataTable
{
    /**
    ...
    public function ajax(): JsonResponse
    {
        ...
        return DataTables::eloquent($video)
            ...
            ->addColumn('action', function ($video) {
                $url =  Archive::where('id', $video->parent_id)->first();
                $response = json_decode($url->raw_response);
                $show = '<a data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="' . __('Show') . '" title="' . __('Show') . '" href="javascript:void(0)" onclick="showVideo(\'' . $response->response->body->video->url . '\', \'' . $video->title . '\')" class="action-icon"><i class="feather icon-eye"></i></a>
                        <div class="modal fade" id="videoModal" tabindex="-1" aria-labelledby="videoModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title text-start fw-bold" id="videoModalLabel"></h5> <!-- Title will be dynamically updated -->
                ...
                $delete = '<form method="POST" action="' . route('admin.features.text-to-video.delete', ['id' => $video->id]) . '" id="delete-video-' . $video->id . '" accept-charset="UTF-8" class="display_inline">
                    ' . csrf_field() . '
                    ' . method_field('DELETE') . '
    ...
    public function getColumns(): array
    {
        return [
            ...
            new Column(['data'=> 'title', 'name' => 'title', 'title' => __('Name'), 'searchable' => true, 'orderable' => true, 'width'=>'45%']),
