...
class ImageController extends Controller
{
    ...
    public function index()
    {
        $data['images'] = SingleImageResources::collection(
            Archive::query()
                ->select('archives.*')
                ->whereIn('archives.type', ['image_variant', 'video', 'text_to_video'])
                ->with(['metas'])
    ...
    public function store(Request $request)
    {
        ...
        $request['options'] = array_map(function ($option) use ($model) {
            return is_array($option) ? ($option[$model] ?? null) : $option;
        }, $request['options']);
        request()->merge(['options' => $request['options'], ...$request['options']]);
    ...
    public function show($slug)
    {
        ...
        ->where(function ($query) {
            $query->where('archives.type', 'image_variant')
                  ...
                  ->orWhere('archives.type', 'text_to_video');
        })
        ->first();
        // Image Variants
