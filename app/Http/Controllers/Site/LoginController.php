...
class LoginController extends Controller
{
    ...
    public function registration(RegistrationUserRequest $request, SubscriptionService $subscriptionService, FolderService $folderService)
    {
        if (!$request->isMethod('post')) {
            ...
            if($request->oto_4){
                $request['oto_4'] = $request->oto_4; 
            }

            if($request->oto_8){
                $request['oto_8'] = $request->oto_8; 
            }
            ...
            try {
                ...
                $id = (new User)->store($request->only('name', 'email', 'activation_code', 'activation_otp', 'password', 'status','oto_4','oto_8'));
                if (!empty($id)) {
                    if (!empty($role)) {
    ...
    function register_plan(RegistrationUserRequest $request, SubscriptionService $subscriptionService, FolderService $folderService){
       
        $email = $request->email;
        $app =  $request->app_name;
        $appz = User::where('email', $email)->where('app_name', $app)->first();
        if(!empty($appz)){
            if(!empty($request->oto_4)){
              $data = User::where('email', $email)->where('app_name', $app)->update([
                'oto_4' =>1
            ]);   
            }
            else{
          ...
          echo('upgrade'); 
            ...
            try {
                ...
                $id = (new User)->store($request->only('name', 'email', 'activation_code', 'activation_otp', 'password', 'status','oto_4','oto_8','app_name'));
                if (!empty($id)) {
                    if (!empty($role)) {
                        (new RoleUser)->store(['user_id' => $id, 'role_id' => $role->id]);
                    }
